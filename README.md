...
## Description
This repository contains code for the ROAM behavioural monitoring framework, a unified computer vision-based pipeline that can be used for real-time monitoring of continuous video sources (CCTV livestreams, drone, and camera trap imagery) for generating metrics such as behavioural activity budget, space use heatmaps, and customizable alert-enabled systems.
...
Before the ROAM pipeline is deployed in real-world scenarios, concordance testing should be carried out to assess the validity of the model of inferring various behavioural states, such as the quantification of loop swimming laps by comparing human-coded observations against the automated, AI-based method. The level of agreement should align with institutional standards (e.g., interobserver reliability of ≥ 85%). Python and R code scripts ("ConcordanceTest.py","Concordance.R") are provided in the Code section to assist users with this step. 
![ModelValidation](./Images/Figure3.png)

As behaviours meet/exceed the criteria for model reliability, the model can be deployed to tally up relevant behaviours such as loop swimming laps in real-time. In addition to the live monitoring interface, raw video frames are translated into structured behavioural observations as an Excel spreadsheet. At the end of the monitoring session, intuitive circular bar charts help to identify circadian patterns of specific behaviours. 
![ZooAI_Outputs](./Images/Figure4.png)
